...
if [ ! -f "/opt/labelberry/menu.sh" ]; then
    echo -e "${YELLOW}menu.sh not found, downloading...${NC}"
    curl -sSL https://raw.githubusercontent.com/Baanaaana/LabelBerry/main/menu.sh -o /opt/labelberry/menu.sh
    chmod +x /opt/labelberry/menu.sh
fi

# Create system-wide command
cat > /usr/local/bin/labelberry-menu << 'EOF'
#!/bin/bash
cd /opt/labelberry && ./menu.sh
EOF
...
echo ""
echo -e "${YELLOW}Try it now: just type 'menu' and press Enter${NC}"