...
import time
STATIC_VERSION = int(time.time()) if os.getenv("DEBUG", "false").lower() == "true" else "1.2"
...
async def list_pis():
    try:
        ...
        label_size_map = {ls.id: ls for ls in label_sizes}
        ...
        for pi in pis:
            ...
            if pi.label_size_id and pi.label_size_id in label_size_map:
                pi_dict["label_size"] = label_size_map[pi.label_size_id].model_dump()
            else:
